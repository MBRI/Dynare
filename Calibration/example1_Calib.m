function [Res]=example1_Calib(Min_Par_Calib,Step_Par_Calib,Max_Par_Calib)
global oo_
Res=struct();
SC=0;
...
close all
SC=SC+1;
Res.(['V' num2str(SC)])=oo_.var;
Res.(['A' num2str(SC)])=oo_.autocorr{1};
Res.(['S' num2str(SC)])=oo_.steady_state;
Res.(['M' num2str(SC)])=oo_.mean;
Res.(['P' num2str(SC)])=Par_Calib;
end
end
end
end
